...
model_catboost_whole = CatBoostRegressor(iterations=1000, learning_rate=0.1, depth=6, verbose=0)
model_catboost_whole.fit(X_train_whole, y_train_whole)

train_predict_catboost_whole = model_catboost_whole.predict(X_train_whole)
test_predict_catboost_whole = model_catboost_whole.predict(X_test_whole)

train_rmse_catboost_whole = np.sqrt(mean_squared_error(y_train_whole, train_predict_catboost_whole))
test_rmse_catboost_whole = np.sqrt(mean_squared_error(y_test_whole, test_predict_catboost_whole))

print(f'CatBoost Train RMSE (Whole Dataset): {train_rmse_catboost_whole:.4f}')
print(f'CatBoost Test RMSE (Whole Dataset): {test_rmse_catboost_whole:.4f}')

plt.figure(figsize=(14, 7))
plt.plot(range(len(y_train_whole)), y_train_whole, label='Actual Train Log Returns (Whole Dataset)')
plt.plot(range(len(y_train_whole)), train_predict_catboost_whole, label='Predicted Train Log Returns (Whole Dataset)')
...
plt.plot(range(len(y_train_whole), len(y_train_whole) + len(y_test_whole)), test_predict_catboost_whole, label='Predicted Test Log Returns (Whole Dataset)')
plt.title(f'CatBoost with LSTM Features - Whole Dataset\nTrain RMSE: {train_rmse_catboost_whole:.4f}, Test RMSE: {test_rmse_catboost_whole:.4f}')
plt.xlabel('Days')
