...
model_xgb_whole = xgb.XGBRegressor(objective='reg:squarederror', eval_metric='rmse')
...
train_rmse_xgb_whole = np.sqrt(mean_squared_error(y_train_whole, train_predict_xgb_whole))
test_rmse_xgb_whole = np.sqrt(mean_squared_error(y_test_whole, test_predict_xgb_whole))

print(f'XGBoost Train RMSE (Whole Dataset): {train_rmse_xgb_whole:.4f}')
print(f'XGBoost Test RMSE (Whole Dataset): {test_rmse_xgb_whole:.4f}')


plt.figure(figsize=(14, 7))
plt.plot(range(len(y_train_whole)), y_train_whole, label='Actual Train Log Returns (Whole Dataset)')
plt.plot(range(len(y_train_whole)), train_predict_xgb_whole, label='Predicted Train Log Returns (Whole Dataset)')
...
plt.plot(range(len(y_train_whole), len(y_train_whole) + len(y_test_whole)), test_predict_xgb_whole, label='Predicted Test Log Returns (Whole Dataset)')
plt.title(f'XGBoost with LSTM Features - Whole Dataset\nTrain RMSE: {train_rmse_xgb_whole:.4f}, Test RMSE: {test_rmse_xgb_whole:.4f}')
plt.xlabel('Days')
